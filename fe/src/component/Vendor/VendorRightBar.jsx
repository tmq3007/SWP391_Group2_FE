...
const VendorRightBar = ({ selectedPage , ooo }) => {
  ...
  const revenueThisMonth = orderItems.filter(item => {
    if (item.isPaid && item.orderItemsPaymentDate) {
  ...
  const handleMonthOrder = () => {
    const monthOrders = orderItems.filter(item => {
      if (item.orderItemsPaymentDate) {
        const orderDate = new Date(item.orderItemsPaymentDate);
        return orderDate.getMonth() === currentMonth && orderDate.getFullYear() === currentYear;
      }
  ...
  const handleYearOrder = () => {
    const yearOrders = orderItems.filter(item => {
      if (item.orderItemsPaymentDate) {
        const orderDate = new Date(item.orderItemsPaymentDate);
  ...
  orderItems.forEach(order => {
    ...
    (order.isPaid) ? monthlyRevenue[month] += order.finalPrice : monthlyRevenue[month] += 0;
  });
  const data = {
  ...
  const paidOnly = orderItems.filter(a => a.isPaid);
  ...
  console.log(b);
  const highestValue = b.reduce((max, order) =>
      order.finalPrice > max.finalPrice ? order : max
  );
  console.log(highestValue);
  const highestOrder = b.reduce((max, order) =>
        ...
        <div>
          <div className="w-full h-full  mb-8 rounded-lg bg-light p-5 md:p-8 border bg-white">
            <div className="mb-7 flex items-center justify-between">
              <span
                  className='font-semibold cursor-pointer hover:text-green-500 hover:border hover:border-solid hover:border-green-400 p-1 rounded-full text-xl'>Summary  <span className={"text-[12px]"}>(Paid order only)</span></span>
                  ...
                  <div className={"flex gap-4"}>
                    <div className={"w-[50%] m-1 flex"}>
                      ...
                      <div className="ml-4 mt-5">
                        ...
                        <p className="text-2xl text-gray-500"><span
                            className="font-bold text-green-500">{highestValue.productName}</span></p>
                        <p className="text-lg text-gray-500">Bring: <span
                            className="font-bold text-green-500">${highestValue.finalPrice}</span></p>
                        <p className="text-lg text-gray-500">Sell unit: <span
                            className="font-bold text-green-500">{highestValue.productQuantity}</span></p>
                      </div>
                    ...
                    <div className={"w-[50%] m-1 flex"}>
                      ...
                      <div className="ml-4 mt-5">
                        ...
                        <Divider/>
                        <p className="text-2xl text-gray-500"><span
                            className="font-bold text-green-500">{highestOrder.productName}</span></p>
                        <p className="text-lg text-gray-500">Bring: <span
                            className="font-bold text-green-500">${highestOrder.finalPrice}</span></p>
                        <p className="text-lg text-gray-500">Sell unit: <span
                            className="font-bold text-green-500">{highestOrder.productQuantity}</span></p>
                      </div>
                    </div>
                  </div>
                </div>
