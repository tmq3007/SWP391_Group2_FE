...
const VendorLeftBar = ({ onSelect, ooo }) => {
  const [user, setUser] = useState((ooo) ? ooo : {});
        ...
        <div className="w-[20%] h-[90vh] border-r-2 border-gray-200 bg-white block mt-7">
          <div className="block border-b border-t border-b-gray-300 pt-5 pl-5 pb-5">
            ...
            <div className={"mt-2 block gap-2"}>
              <div className={"flex gap-2"}>
                ...
                <Typography>{(user) ? (user.user.firstName + user.user.lastName) : "Shop owner name"}</Typography>
              </div>
              <div className={"flex gap-2"}>
                <EmailIcon className={"text-green-500"}/>
                <Typography>{(user) ? (user.user.email) : "Shop owner name"}</Typography>
              </div>
              <div className={"flex gap-2"}>
                <LocalPhoneIcon className={"text-green-500"}/>
                <Typography>{(user) ? (user.user.phone) : "Shop owner name"}</Typography>
              </div>
            </div>
          </div>
          ...
          <div className="block pt-5 pl-3 pb-5">
            <List sx={{width: "100%", bgcolor: "background.paper"}}>
              ...
              <ListItem className="shadow-md hover:bg-slate-200" button="true" onClick={() => onSelect(4)}>
                <ListItemAvatar>
                  <Avatar>
                    <StarsRoundedIcon/>
                  </Avatar>
              ...
              <ListItem className="shadow-md hover:bg-slate-200" button="true" onClick={() => onSelect(5)}>
                <ListItemAvatar>
                  <Avatar>
                    <CategoryRoundedIcon/>
                  </Avatar>
